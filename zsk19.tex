...
Option prices $B^{\tau,i}_t$ are computed according to the Black-Scholes formula where the implied volatility is a quadratic function of the strike price relative to the spot price. The shape of the function, depicted in Picture XX, has been estimated using 110 observations of actual option prices on ???market. As the risk-less rate ??? was taken. 

\pgfimage[width=7cm]{smile}



Model for $(X,Y)$. As both the correlations of $X_t,Y_t$ with $P_t$, $P_{t-1}$  and correlations $\Delta X_t,\Delta Y_t$ with $\Delta P_t$, $\Delta P_{t-1}$ are insignificant, we model $X,Y$ alone, without dependence on $P$. The time series-- plots and xy--plots of $X,Y$ and their differences can be seen in Figures:
...
As it could have been expected, the values of $X$ and $Y$ ``go along''. As the ADF tests rejected unit root hypothesis for both the series, we chose VAR model with a single lag to fit their time dependence:
$$
